...
public class GroupedCraftingEmiRecipe extends BasicEmiRecipe {
	...
	private GroupedCraftingEmiRecipe(Identifier id, GroupedCraftingRecipe recipe, boolean indexInputs, boolean indexOutputs) {
	...
	public static List<GroupedCraftingEmiRecipe> ofGroupedUsageAndFocusedRecipes(RecipeHolder<GroupedCraftingRecipe> recipeHolder) {
		...
		recipes.add(new GroupedCraftingEmiRecipe(recipeHolder.id(), recipe, true, false));
		for (int i = 0; i < recipe.getVariants().size(); i++) {
			int variantIndex = i;
			Identifier id = recipeHolder.id().withPath(path -> path + "/" + variantIndex);
