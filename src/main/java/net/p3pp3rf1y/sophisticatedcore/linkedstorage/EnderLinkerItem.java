...
public class EnderLinkerItem extends ItemBase {
	...
	public InteractionResult onItemUseFirst(ItemStack linker, UseOnContext context) {
		...
		return switch (tryLinkBlock(player, linker, context.getLevel(), context.getClickedPos())) {
	...
	private static LinkAttempt tryLinkBlock(Player player, ItemStack linker, Level level, BlockPos pos) {
		BlockEntity blockEntity = level.getBlockEntity(pos);
		if (!(blockEntity instanceof ILinkedStorageBlockEndpoint endpoint)) {
			return LinkAttempt.PASS;
		}
	...
	private static LinkedStorageService.LinkResult linkWithFeedback(Player player, ItemStack linker, @Nullable BlockPos blockPos,
		...
		if (LinkedStorageStackData.getPendingCraft(linker) != null && !EnderLinkerEndpointRecipe.finalizePendingCraftLinker(level, linker)) {
	...
	public void inventoryTick(ItemStack stack, Level level, Entity entity, int slotId, boolean isSelected) {
		if (level instanceof ServerLevel serverLevel && LinkedStorageStackData.getPendingCraft(stack) != null
				&& !EnderLinkerEndpointRecipe.finalizePendingCraftLinker(serverLevel, stack)) {
			throw new IllegalStateException("Could not finalize pending Ender Linker craft claim");
		}
	}
	...
	public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag tooltipFlag) {
		...
		tooltip.add(TranslationHelper.INSTANCE.translItemTooltip(this, boundLinker ? "bound" : "blank").withStyle(ChatFormatting.DARK_GRAY));
		if (state == EnderLinkerStackState.TARGET) {
			EnderLinkerTargetData target = LinkedStorageStackData.getLinkerTarget(stack);
			if (!target.groupName().getString().isEmpty()) {
				tooltip.add(TranslationHelper.INSTANCE
			...
			tooltip.add(TranslationHelper.INSTANCE.translItemTooltip(this, "linker_pending").withStyle(ChatFormatting.GRAY));
		}
	}
	...
	private static boolean isPendingLinker(ItemStack stack, EnderLinkerStackState state) {
		...
		return stack.getItem() instanceof EnderLinkerItem && state == EnderLinkerStackState.PENDING_CRAFT && pendingCraft != null
				&& pendingCraft.resolvesToLinkerTarget();
	}
