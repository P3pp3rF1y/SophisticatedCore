...
public class EnderLinkerEndpointRecipe extends CustomRecipe {
	public static final EnderLinkerEndpointRecipe INSTANCE = new EnderLinkerEndpointRecipe(CraftingBookCategory.MISC);
	public static final MapCodec<EnderLinkerEndpointRecipe> MAP_CODEC = MapCodec.unit(INSTANCE);
	public static final StreamCodec<RegistryFriendlyByteBuf, EnderLinkerEndpointRecipe> STREAM_CODEC = StreamCodec.unit(INSTANCE);
	public static final RecipeSerializer<EnderLinkerEndpointRecipe> SERIALIZER = new RecipeSerializer<>(MAP_CODEC, STREAM_CODEC);
	...
	public ItemStack assemble(CraftingInput input) {
		return getInputs(input.size(), input::getItem).map(inputs -> {
			ItemStack result = inputs.plan().returnsLinker() ? inputs.linker().copyWithCount(1) : inputs.endpoint().copyWithCount(1);
	...
	public static void finalizeTransferredCraftResult(ServerLevel level, ItemStack transferredStack, InventoryHandler storageInventory,
			Inventory playerInventory) {
		EnderLinkPendingCraftData pendingCraft = transferredStack.get(ModCoreDataComponents.ENDER_LINK_PENDING_CRAFT);
		if (pendingCraft == null || pendingCraft.claimId() == null) {
			return;
		}
		ItemStack result = findTransferredCraftResult(pendingCraft.claimId(), storageInventory.size(), storageInventory::getStackInSlot);
		if (result.isEmpty()) {
			result = findTransferredCraftResult(pendingCraft.claimId(), playerInventory.getContainerSize(), playerInventory::getItem);
		}
		if (!result.isEmpty()) {
	...
	public static void finalizeDeliveredCraftResult(ServerLevel level, ItemStack craftedStack, Inventory playerInventory, ItemStack carriedStack) {
		...
		ItemStack result = hasClaim(pendingCraft.claimId(), carriedStack)
				? carriedStack
				: findTransferredCraftResult(pendingCraft.claimId(), playerInventory.getContainerSize(), playerInventory::getItem);
		if (result.isEmpty()) {
	...
	private static ItemStack findTransferredCraftResult(UUID claimId, int inventorySize, IntFunction<ItemStack> getItem) {
