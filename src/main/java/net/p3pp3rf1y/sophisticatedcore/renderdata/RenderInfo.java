...
public abstract class RenderInfo {
	...
	static {
		RENDER_DATA_TYPES = Map.of(CookingUpgradeRenderData.TYPE.getName(), CookingUpgradeRenderData.TYPE, JukeboxUpgradeRenderData.TYPE.getName(),
				JukeboxUpgradeRenderData.TYPE);
	}
	...
	private static <T extends IUpgradeRenderData> boolean isUpgradeDataValid(IStorageWrapper storageWrapper, Level level, UpgradeRenderDataType<?> type,
		...
		return UpgradeRenderDataValidatorRegistry.getValidator(typedType)
				.map(validator -> validator.isValid(storageWrapper, level, typedType.cast(data).orElseThrow())).orElse(true);
	}
	...
	public static class ItemDisplayRenderInfo {
		...
		public static ItemDisplayRenderInfo deserialize(CompoundTag tag) {
			...
			if (tag.getTagType(INACCESSIBLE_SLOTS_TAG) == Tag.TAG_INT_ARRAY) {
				...
				inaccessibleSlots = NBTHelper.getCollection(tag, INACCESSIBLE_SLOTS_TAG, Tag.TAG_INT, t -> Optional.of(((IntTag) t).getAsInt()), ArrayList::new)
						.orElseGet(ArrayList::new); // TODO remove this legacy support in the future
			}
