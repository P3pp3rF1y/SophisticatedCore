...
public abstract class RenderInfo {
	...
	private void deserializeUpgradeData(CompoundTag renderInfoTag) {
		...
		upgrades.keySet().forEach(key -> {
			if (CLIENT_DATA_TYPES.containsKey(key)) {
				...
				clientData.put(upgradeClientDataType, upgradeClientDataType.deserialize(upgrades.getCompoundOrEmpty(key)));
			}
		});
	}
	...
	private boolean isDifferentFluidOrFillStep(@Nullable IRenderedTankUpgrade.TankRenderInfo existingInfo,
		...
		if (!FluidStack.isSameFluidSameComponents(existingInfo.getFluid().get(), newInfo.getFluid().get())) {
			return true;
		}
		int fillSteps = 20;
	...
	private void deserializeTanks(CompoundTag renderInfoTag) {
		...
		for (int i = 0; i < tanks.size(); i++) {
			...
			tankRenderInfos.put(tank.getString(TANK_POSITION_TAG).map(s -> TankPosition.valueOf(s.toUpperCase(Locale.ROOT))).orElse(TankPosition.LEFT),
					IRenderedTankUpgrade.TankRenderInfo.deserialize(tank.getCompoundOrEmpty(TANK_INFO_TAG)));
		}
	}
	...
	private void serializeTank(TankPosition tankPosition, IRenderedTankUpgrade.TankRenderInfo tankRenderInfo) {
		...
		for (int i = 0; i < tanks.size(); i++) {
			CompoundTag tank = tanks.getCompoundOrEmpty(i);
			if (tank.getString(TANK_POSITION_TAG).map(positionString -> positionString.equals(tankPosition.getSerializedName())).orElse(false)) {
	...
	public static class ItemDisplayRenderInfo {
		...
		public static ItemDisplayRenderInfo deserialize(CompoundTag tag) {
			...
			if (tag.contains(DisplayItem.ITEM_TAG)) {
				...
				List<DisplayItem> items = NBTHelper
						.getCollection(tag, ITEMS_TAG, stackTag -> Optional.of(DisplayItem.deserialize((CompoundTag) stackTag)), ArrayList::new)
	...
	public static class DisplayItem {
		...
		private static DisplayItem deserialize(CompoundTag tag) {
			return new DisplayItem(tag.getCompound(ITEM_TAG).flatMap(NBTHelper::deserializeStackFromTag).orElse(ItemStack.EMPTY), tag.getIntOr(ROTATION_TAG, 0),
					tag.getIntOr(SLOT_INDEX_TAG, 0), tag.getString(DISPLAY_SIDE_TAG).map(DisplaySide::fromName).orElse(DisplaySide.FRONT),
					tag.getIntOr(Z_OFFSET_TAG, 0));
		}
