...
public class StorageSoundHandler {
	...
	public static void playStorageSound(SoundEvent soundEvent, UUID storageUuid, int entityId) {
		...
		if (player != null && player.getId() == entityId) {
			playStorageSound(storageUuid, new PlayerDirectStorageSoundInstance(soundEvent, player, level.getRandom(), volume, pitch));
			return;
		}
		playStorageSound(storageUuid, new EntityBoundSoundInstance(soundEvent, SoundSource.RECORDS, volume, pitch, entity, level.getRandom().nextLong()) {
			@Override
			public void tick() {
				super.tick();
