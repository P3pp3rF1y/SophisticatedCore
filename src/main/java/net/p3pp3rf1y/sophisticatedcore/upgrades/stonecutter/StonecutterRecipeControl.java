...
public class StonecutterRecipeControl extends WidgetBase {
	...
	protected void renderBg(GuiGraphics guiGraphics, Minecraft minecraft, int mouseX, int mouseY) {
		...
		GuiHelper.blit(guiGraphics, x + getListCenteredX(26), y + INPUT_SLOT_HEIGHT + SPACING + LIST_BACKGROUND.getHeight() + SPACING,
	...
	private void drawRecipesItems(GuiGraphics guiGraphics, int listInnerLeftX, int top, int recipeIndexOffsetMax) {
		...
		for (int displayIndex = recipeIndexOffset; displayIndex < recipeIndexOffsetMax && displayIndex < sortedRecipeIndexes.size(); ++displayIndex) {
			...
			int l = j / 4;
			int i1 = top + l * 18 + 2;
			GuiHelper.renderItemInGUI(guiGraphics, minecraft, list.get(sortedRecipeIndexes.get(displayIndex)).getResultItem(minecraft.level.registryAccess()),
					k, i1);
		}
	}
		...
		if (hasItemsInInputSlot) {
			...
			for (int displayIndex = recipeIndexOffset; displayIndex < k && displayIndex < sortedRecipeIndexes.size(); ++displayIndex) {
				...
				if (mouseX >= recipeLeftX && mouseX < recipeLeftX + 16 && mouseY >= k1 && mouseY < k1 + 18) {
					renderTooltip(guiGraphics, list.get(sortedRecipeIndexes.get(displayIndex)).getResultItem(minecraft.level.registryAccess()), mouseX, mouseY);
				}
			}
		}
	}
	...
	private void openLargeResultSelector() {
		...
		List<StonecutterRecipe> recipes = container.getRecipeList();
		for (int i : getSortedRecipeIndexes()) {
			results.add(new LargeResultSelector.ResultEntry(i, recipes.get(i).getResultItem(minecraft.level.registryAccess()), container.isRecentResult(i)));
		}
	...
	private void setFirstVisibleRow(int firstVisibleRow) {
		...
		sliderProgress = hiddenRows == 0 ? 0 : (float) clampedFirstVisibleRow / hiddenRows;
	}
