...
public class TankUpgradeConfig {
	...
	public TankUpgradeConfig(ModConfigSpec.Builder builder) {
		...
		capacityPerSlotRow = builder.comment("Capacity in mB the tank upgrade will have per row of storage slots").defineInRange("capacityPerSlotRow", 4000,
				500, 20000);
		stackMultiplierRatio = builder.comment(
				...
				.defineInRange("stackMultiplierRatio", 1D, 0D, 5D);
		autoFillDrainContainerCooldown = builder.comment(
		...
		maxInputOutput = builder.comment(
