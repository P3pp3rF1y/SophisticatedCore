...
public class CraftingSpecReiDisplay extends DefaultCraftingDisplay {
	...
	public CraftingSpecReiDisplay(CraftingDisplaySpec spec, List<CraftingDisplayVariant> variants) {
		super(getInputs(spec, variants), List.of(EntryIngredients.ofItemStacks(spec.getOutputStacks(variants))), Optional.of(displayId(spec)));
		this.spec = spec;
	}

	private static ResourceLocation displayId(CraftingDisplaySpec spec) {
		return spec.replacedRecipeIds().size() == 1 ? spec.replacedRecipeIds().iterator().next() : spec.id();
	}
