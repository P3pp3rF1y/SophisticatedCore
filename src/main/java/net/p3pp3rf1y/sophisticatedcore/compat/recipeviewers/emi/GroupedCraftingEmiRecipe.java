...
public class GroupedCraftingEmiRecipe extends BasicEmiRecipe {
	...
	private GroupedCraftingEmiRecipe(ResourceLocation id, GroupedCraftingRecipe recipe, IntPredicate inputIndexFilter, boolean indexOutputs) {
		...
		recipeHolder = new RecipeHolder<>(id, recipe);
	...
	public static List<GroupedCraftingEmiRecipe> ofGroupedUsageAndFocusedRecipes(RecipeHolder<GroupedCraftingRecipe> recipeHolder) {
		...
		recipes.add(new GroupedCraftingEmiRecipe(recipeHolder.id(), recipe, inputIndex -> !isBroadFixedInput(recipe, inputIndex), false));
		addFixedInputFocusedUsageRecipes(recipeHolder.id(), recipe, recipes);
		for (int i = 0; i < recipe.getVariants().size(); i++) {
			int variantIndex = i;
			ResourceLocation id = recipeHolder.id().withPath(path -> path + "/" + variantIndex);
			GroupedCraftingRecipe narrowedRecipe = new GroupedCraftingRecipe(id, recipe.getDisplayWidth(), recipe.getDisplayHeight(), recipe.getFixedInputSlots(), List.of(recipe.getVariants().get(i)), recipe.getResultMatcher());
