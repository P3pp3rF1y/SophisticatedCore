...
public class JeiRecipeDisplayGenerator implements IRecipeDisplayGenerator<CraftingRecipe> {
	...
	public IRecipeDisplayBuilder smithing(Optional<Ingredient> template, Optional<Ingredient> base, Optional<Ingredient> addition, ItemStack result) {
		return id -> acceptSmithing(new RecipeHolder<>(id, new SmithingTransformRecipe(template, base.orElseThrow(), addition, new TransmuteResult(result.getItemHolder(), result.getCount(), result.getComponentsPatch()))));
	}
