...
public class CraftingUpgradeTweakProvider implements CraftingGridProvider {
	...
	private class StorageCraftingGrid implements CraftingGrid, GridGuiSettings {
		...
		public int getGridWidth(Player player, AbstractContainerMenu menu) {
			return menu instanceof StorageContainerMenuBase<?> storageMenu ? getCraftingGridWidth(storageMenu) : 0;
		}

		@Override
		public int getGridHeight(Player player, AbstractContainerMenu menu) {
			return menu instanceof StorageContainerMenuBase<?> storageMenu ? getCraftingGridHeight(storageMenu) : 0;
		}
	...
	private static class StorageCraftingGridRefillHandler implements GridRefillHandler<AbstractContainerMenu> {
		...
		public void refillRecipe(CraftingGrid grid, Player player, AbstractContainerMenu menu, RecipeHolder<?> recipeHolder, boolean stack) {
			...
			int gridWidth = grid.getGridWidth(player, menu);
			...
			if (recipeMapper.getIngredients(recipe).size() > gridSize) {
				return;
			}

			int operations = 0;
			do {
				Optional<Map<Integer, IngredientToken>> matrixDiff = getMatrixDiff(craftMatrix, recipe, recipeMapper, gridWidth,
						operation.getIngredientTokens());
		...
		@SuppressWarnings({"rawtypes", "unchecked"})
		private static Optional<Map<Integer, IngredientToken>> getMatrixDiff(Container craftMatrix, Recipe<?> recipe, RecipeMapper recipeMapper, int gridWidth,
			...
			return getCraftMatrix(storageContainer).map(craftMatrix -> {
				...
				if (itemStack.isEmpty()) {
					...
					for (int i = start; i < start + size; ++i) {
						int slotIndex = menu.getSlot(i).getSlotIndex();
