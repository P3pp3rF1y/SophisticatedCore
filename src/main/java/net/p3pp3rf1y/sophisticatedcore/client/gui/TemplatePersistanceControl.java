...
public class TemplatePersistanceControl extends CompositeWidgetBase<WidgetBase> {
	...
	public static final ButtonDefinition EXPORT_TEMPLATE = new ButtonDefinition(Dimension.SQUARE_16, DEFAULT_BUTTON_BACKGROUND, DEFAULT_BUTTON_HOVERED_BACKGROUND, EXPORT_FOREGROUND,
			new TranslatableComponent(TranslationHelper.INSTANCE.translSettingsButton("export_template")));
	...
	protected TemplatePersistanceControl(Position position, TemplatePersistanceContainer container) {
		...
		exportTemplateButton.setTooltip(List.of(
				new TranslatableComponent(TranslationHelper.INSTANCE.translSettingsButton("export_template"),
						new TranslatableComponent(TranslationHelper.INSTANCE.translSettingsButton("export_template.enter_file_name")).withStyle(ChatFormatting.GREEN)),
				new TranslatableComponent(TranslationHelper.INSTANCE.translSettingsButton("export_template.additional_info")).withStyle(ChatFormatting.ITALIC, ChatFormatting.DARK_GRAY))
		);
	...
	private void setLoadTooltip() {
		if (container.getLoadSlot() == -1) {
			...
			tooltip.add(new TranslatableComponent(TranslationHelper.INSTANCE.translSettingsButton("load_template"), container.getLoadSlotTooltipName().withStyle(ChatFormatting.GREEN)));
			container.getLoadSlotSource().ifPresent(source -> tooltip.add(new TranslatableComponent(TranslationHelper.INSTANCE.translSettingsButton("load_template.source"), new TextComponent(source).withStyle(ChatFormatting.ITALIC, ChatFormatting.GRAY)).withStyle(ChatFormatting.ITALIC, ChatFormatting.DARK_GRAY)));
			tooltip.add(new TranslatableComponent(TranslationHelper.INSTANCE.translSettingsButton("load_template.controls")).withStyle(ChatFormatting.ITALIC, ChatFormatting.DARK_GRAY));
			if (container.templateHasTooManySlots()) {
				tooltip.add(new TranslatableComponent(TranslationHelper.INSTANCE.translSettingsButton("load_template.too_many_setting_slots")).withStyle(ChatFormatting.RED));
			}
	...
	private void setSaveTooltip() {
		saveTemplateButton.setTooltip(List.of(
				new TranslatableComponent(TranslationHelper.INSTANCE.translSettingsButton("save_template"), container.getSaveSlotTooltipName().withStyle(ChatFormatting.GREEN)),
