...
public class Label extends WidgetBase {
	...
	protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (isTextTruncated()) {
			guiGraphics.drawString(minecraft.font, getTruncatedText(), x, y, color, false);
		} else {
	...
	public void renderTooltip(Screen screen, GuiGraphics guiGraphics, int mouseX, int mouseY) {
		if (isTextTruncated() && isMouseOver(mouseX, mouseY)) {
			guiGraphics.renderTooltip(screen.getFont(), List.of(labelText), Optional.empty(), mouseX, mouseY);
		}
	}
