...
public class ColorPicker extends CompositeWidgetBase<WidgetBase> {
	...
	private void addDefaultColorButtons() {
		for (DyeColor value : DyeColor.values()) {
			int color = ColorHelper.opaque(ColorHelper.getColor(value.getTextureDiffuseColors()));
	...
	private static class ColorGradientArea extends WidgetBase {
		...
		public void setHue(float hue) {
			...
			float value = hsv[2];
			color = ColorHelper.opaque(Mth.hsvToRgb(hue, saturation, value));
		...
		protected void renderBg(GuiGraphics guiGraphics, Minecraft minecraft, int mouseX, int mouseY) {
			...
			for (int i = 0; i < getWidth(); i++) {
				for (int j = 0; j < getHeight(); j++) {
					...
					int color = FastColor.ARGB32.color(255, (int) ((1 - verticalFactor) * ((1 - horizontalFactor) * 255 + red * horizontalFactor)),
	...
	private static class RainbowSlider extends WidgetBase {
		...
		protected void renderBg(GuiGraphics guiGraphics, Minecraft minecraft, int mouseX, int mouseY) {
			for (int i = 0; i < getHeight(); i++) {
				...
				int color = ColorHelper.opaque(Mth.hsvToRgb(renderedHue, 1, 1));
