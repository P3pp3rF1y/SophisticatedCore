...
public class BlockHighlightRenderHelper {
	...
	public static final RenderPipeline THICK_HIGHLIGHT_PIPELINE = RenderPipeline.builder(RenderPipelines.MATRICES_COLOR_FOG_SNIPPET)
			...
			.withDepthTestFunction(DepthTestFunction.LEQUAL_DEPTH_TEST).withDepthWrite(true).withLocation(SophisticatedCore.getRL("pipeline/outline_quads"))
			.build();

	public static final RenderType THICK_HIGHLIGHT_QUADS = RenderType.create("storage_outline_quads", 1536, THICK_HIGHLIGHT_PIPELINE, RenderType.CompositeState
			.builder().setLayeringState(RenderType.VIEW_OFFSET_Z_LAYERING).setOutputState(RenderType.ITEM_ENTITY_TARGET).createCompositeState(false));
